...
`CLOUDSDK_AUTH_ACCESS_TOKEN_FILE`, `aws` → `AWS_SHARED_CREDENTIALS_FILE`. Each
value file gets a `<kind>.expiry` sidecar (unix timestamp, written last);
`dcx-shim` reads it and blocks before the API does. Preserve both properties in
any change here.
